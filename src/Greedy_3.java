...
public class Greedy_3 {
     ...
     * 19399 ATM
     *
     * 인하은행에는 ATM이 1대밖에 없다 -> 동시에 작동 못한다. -> 활동선택문제 -> 그리디알고리즘
     * 대기시간을 줄이려면 앞 사람이 일찍 끝내야 그만큼 대기시간을 줄일 수 있다.
     * 그리고 각각의 사람이 돈을 인출할 때에는 다른 사람이 개입할 수 없으므로
     * 그리디 알고리즘의 조건인 독립성과 전체 최적해가 부분 최적해를 만족함 둘 다 만족한다.
     *
     * 문제
    ...
    public int solution(int N, ArrayList<Integer> list) {

        Collections.sort(list);

        ArrayList<Integer> sumTimeList = new ArrayList<>();
        ...
        int sum = sumTimeList.stream().mapToInt(i -> i.intValue()).sum();

        return sum;
    }

    public static void main(String[] args) throws IOException {
        Greedy_3 main = new Greedy_3();
