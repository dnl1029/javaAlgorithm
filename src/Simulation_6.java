...
public class Simulation_6 {
    ...
    public int solution(int[][] intarr, int N, int M) {
        ...
        for(int i=0; i<N; i++) {
            tempMap.put(intarr[i][0],intarr[i][1]*10000*10000+intarr[i][2]*10000+intarr[i][3]);
        }

        for(int i=1;i<=N;i++) {
            int temp = 1;
            for(int j=1; j<=N; j++) {
                if(tempMap.get(i)<tempMap.get(j)) {
                    temp++;
                }
                resultMap.put(i,temp);
            }
        }
        answer = resultMap.get(M).intValue();
        return answer;
    }

    public static void main(String[] args) throws IOException {
        Simulation_6 main = new Simulation_6();
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        StringTokenizer st1 = new StringTokenizer(br.readLine());
        int N = Integer.parseInt(st1.nextToken());
        int M = Integer.parseInt(st1.nextToken());
        int[][] intarr = new int[N][4];
        ...
        br.close();
        System.out.println(main.solution(intarr,N,M));
    }
}
